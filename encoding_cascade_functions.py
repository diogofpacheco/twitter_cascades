...
def calculateNetworkMetrics(G, root, nodes):
    ...
    return {
        'depth': [max(short_paths_from_root[:(idx+1)]) for idx in range(len(short_paths_from_root))],
        'depth_max': max(depth),
...
def calculate_recent_root_distances(egde_list):
    ...
    return pd.Series({
        'recent_edge': cascade_vector[0],
        'root_edge': cascade_vector[-1]
    })
